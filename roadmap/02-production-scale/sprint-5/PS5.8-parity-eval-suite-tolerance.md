...
- [x] Manual (with NIM up): both arms `valid_for_parity=true` on ≥2 cases → `comparable` summary.

- [x] Manual (with NIM down): GPU arm `invalid_fallback` or `invalid_gpu_unavailable` — **no** false “100% match”.
