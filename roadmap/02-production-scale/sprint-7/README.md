...
- [ ] PS7.1: stage GKE + Helm with `values-gcp-stage.yaml`; demo A/B from runbook.
- [ ] PS7.2: budget alert in project + runbook entry (dated drill).
- [ ] PS7.4: `docs/monitoring-production-analysis.md` with recommendations.
- [ ] PS7.5: READMEs for BL-002 folder list (minimum: `data/`, `kb/`, `evals/`, `infra/` subfolders).

**PS7b / optional (explicit defer OK)**

- [ ] PS7.3: Variant A graph worker + test kill worker → resume.
