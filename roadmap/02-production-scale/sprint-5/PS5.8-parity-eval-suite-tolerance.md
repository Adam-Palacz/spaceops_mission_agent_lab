...
- [ ] Tolerance doc: exact match (escalation yes/no, policy deny) vs allowed drift (wording, latency p95 band).
- [ ] Parity runner: two explicit runs — `LLM_BACKEND=openai` and `LLM_BACKEND=gpu` — merge by case id.
- [ ] Runner reads gateway metadata from eval harness (or structured log tail) — not inference from env alone.
- [ ] CI: `.github/workflows/backend-parity.yml` — `workflow_dispatch` + schedule; **soft** gate (PS4.7).
- [ ] Optional LangSmith/MLflow export for trends — does not replace fixture evals.
- [ ] Sample report: `evals/reports/sample_backend_parity_report.json` includes examples for
...
- [ ] `evals/backend_parity.py` — `--backend openai|gpu`, emits schema above.
- [ ] Cases: must-escalate + citation-present (from existing suites).
- [ ] `docs/evals_backend_parity.md` — promotion vs resilience distinction.
- [ ] Unit test: single-call, `fallback_used=true`, all `backend_actual=openai` →
      `parity_status=invalid_fallback` (not `invalid_mixed_backends`).
- [ ] Unit test: **mixed run** — call 0 `backend_actual=gpu`, call 1 `backend_actual=openai`,
      `fallback_used=true` on call 1 → `parity_status=invalid_mixed_backends` (priority 1).
- [ ] Unit test: OpenAI baseline arm served only by `cursor_sh`, GPU arm is `comparable` →
      OpenAI arm `parity_status=invalid_backend_mismatch` and `gpu_promotion: blocked`.
- [ ] Unit test: required case is missing either arm → `gpu_promotion: blocked`.

---

## Test / acceptance

- [ ] Fixture-based CI test: no live LLM; validates schema and `invalid_fallback` handling.
...
- [ ] `ci_gating_policy.md` updated: parity job is **non-blocking** on PRs; promotion requires
